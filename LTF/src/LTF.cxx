// (c) MPI Munich. Mar 2020
// Author D. Britzger
// --------------------------------------------------------------- //
/*
 * LTF
 * Linear template fit
 * implemented as header-only library using the Eigen library
 *
 */
// --------------------------------------------------------------- //
...
Eigen::VectorXd LTF::LiTeFit::ComputeNewtonEstimator(int mPolN, int nInfrc, const Eigen::VectorXd& ain) /*const*/ {
   ...
   // cout<<"++++++++++++++++++++++++ "<<endl;
   //cout<<"Hesse: "<<endl<<Hesse<<endl; 
   // cout<<"  DIFF Newton:        "<< diff.transpose() <<endl;
   // cout<<"  DIFF Newton approx: "<< diffApprox.transpose() <<endl
   //cout<<" *** delta-Chi2 approx: "<< chi2aprox<< endl;
...
double LTF::LiTeFit::DoLiTeFit(int mPolN, int mOrdInfrc,  const Eigen::VectorXd& aexp) {
   ...
   if ( DoTests ) {
      ...
      constexpr bool DoHelmertWolf = false;
      if ( DoHelmertWolf ) {
         cout<<"----------------- D   : "<<endl<<D<<endl<<"-------------------------------------------------------------"<<endl;
         cout<<"----------------- Dinv: "<<endl<<Dinv<<endl<<"-------------------------------------------------------------"<<endl;
         Eigen::MatrixXd blockD = D.block(1,1,nParEps-1,nParEps-1);
         Eigen::MatrixXd blockB = D.block(0,1,1,nParEps-1);
         Eigen::MatrixXd blockA = D.block(0,0,1,1);
         cout<<"----------------- blockD: "<<endl<<blockD<<endl<<"-------------------------------------------------------------"<<endl;
         cout<<"----------------- blockB: "<<endl<<blockB<<endl<<"-------------------------------------------------------------"<<endl;
         cout<<"----------------- blockA: "<<endl<<blockA<<endl<<"-------------------------------------------------------------"<<endl;
         cout<<"----------------- HelmertWolf: "<<endl<<(blockD-blockB.transpose()*blockA.inverse()*blockB).inverse()<<endl<<"-------------------------------------------------------------"<<endl; // eigen stops (if S=0?)
   
         Eigen::MatrixXd unit = Eigen::MatrixXd::Identity(Sys.size(),Sys.size());
         //longmatrix = S.transpose()*W*S - S.transpose()*W*Y*(Y.transpose()*W*Y).inverse()*Y.transpose()*W*S;

         Eigen::MatrixXd longmatrix2 = S.transpose()*W*S + unit - S.transpose()*W*Ytil*(blockA).inverse()*Ytil.transpose()*W*S;
         cout<<" ----------------- longmatrix.inverse: "<<endl<<longmatrix2.inverse()<<endl; // eigen stops (if S=0?)

         // //Eigen::MatrixXd longmatrix = S.transpose()*W*S - S.transpose()*W*Ytil*(blockA).inverse()*Ytil.transpose()*W*S;
         // //Eigen::MatrixXd longmatrix = longmatrix2 - unit;
         // Eigen::MatrixXd longmatrix = S.transpose()*W*S + S.transpose()*W*Ytil*(blockA).inverse()*Ytil.transpose()*W*S;

         // double trace = longmatrix.trace();
         // cout<<"trace A: "<<unit.trace()<<endl;
         // cout<<"trace B: "<<longmatrix.trace()<<endl;
         // cout<<"trace  : "<<trace<<endl;
         // Eigen::MatrixXd KMinv = unit - 1./(trace) * longmatrix;
         // cout<<" ----------------- ken-miller inverse of block: "<<endl<<KMinv<<endl;

         // {
         //    Eigen::MatrixXd A = (1./trace)*unit;
         //    Eigen::MatrixXd B = (1./trace)*longmatrix;
         ...
         //    double g = (B*A.inverse()).trace();
         //    cout<<"g: "<<g<<endl;
         //    cout<<" ----------------- ken-miller full: "<<endl<< (1./trace) * (A.inverse() - 1./(1+g)*A.inverse()*B*A.inverse() ) <<endl;

   
         //    cout<<" -----------------  (1+long)^-1: "<<endl<< (unit+longmatrix).inverse()<<endl;
         // }

         // { // ken miller 1+STWS
         //    Eigen::MatrixXd A = unit;
         //    Eigen::MatrixXd B = S.transpose()*W*S;
         //    cout<<" -------------------------- STWS inverse "<<endl<< (A+B).inverse()<<endl;
         //    double g = (B*A.inverse()).trace();
         //    cout<<"trace A: "<<A.trace()<<endl;
         //    cout<<"trace B: "<<B.trace()<<endl;
         //    //cout<<"rank B: "<<B.rank()<<endl;
         ...
         //    cout<<" ----------------- ken-miller STWS: "<<endl<<  (A.inverse() - 1./(1+g)*A.inverse()*B*A.inverse() ) <<endl;
         // }


         {// woodburry (1+STWS)^-1 = 1 - ST(V+SSt)^-1S
            Eigen::MatrixXd A = unit;
            Eigen::MatrixXd B = S.transpose()*W*S;
            cout<<" -------------------------- STWS inverse "<<endl<< (A+B).inverse()<<endl;
            cout<<" ----------------- woodburry, duncan44 STWS: "<<endl<<  (unit - S.transpose()*(W.inverse()+S*S.transpose()).inverse()*S) <<endl;
         }


         Eigen::MatrixXd L           = S.transpose()*W*S + unit - S.transpose()*W*Ytil*(Ytil.transpose()*W*Ytil).inverse()*Ytil.transpose()*W*S;
         cout<<"-------------- L :"<<endl<<L<<endl;
         cout<<"-------------- L inverse :"<<endl<<L.inverse()<<endl;
         //Eigen::MatrixXd LinvNeu    = unit - S.transpose()*(W.inverse()  + S*S.transpose() - 2.* (Ytil*Ytil.transpose()) ).inverse()*S;
         Eigen::MatrixXd LinvNeu    = unit - S.transpose()*(W.inverse()  + S*S.transpose() + 1. * (Ytil*Ytil.transpose() ) ).inverse()*S;
         cout<<"-------------- LinvNeu :"<<endl<<LinvNeu<<endl;
         cout<<endl<<"TEST NEU ::"<<endl;
         cout<<LinvNeu * L <<endl;
         cout<<endl<<"TEST Alt ::"<<endl;
         cout<<L.inverse() * L <<endl;

         cout<<"Null test:" <<endl;
         cout << Ytil.transpose()*W*Ytil - ((Ytil.transpose()*W*Ytil).inverse()).inverse()<<endl;
         cout<<"Null test 0.inverse():" <<endl;
         cout << (Ytil.transpose()*W*Ytil - ((Ytil.transpose()*W*Ytil).inverse()).inverse() ).inverse()<<endl;
         cout<<"YTWY: :      "<<(Ytil.transpose()*W*Ytil) <<endl;
         cout<<"YTWY inve: : "<<(Ytil.transpose()*W*Ytil).inverse() <<endl;

         //cout<<"----------------- HelmertWolf: "<<endl<<(blockD-blockB.transpose()*blockA.inverse()*blockB).inverse()<<endl<<"-------------------------------------------------------------"<<endl;

         cout<<" LinvNeu    * orig: "<<endl<< (blockD-blockB.transpose()*blockA.inverse()*blockB) * LinvNeu <<endl;
         cout<<" LinvDirekt * orig: "<<endl<< (blockD-blockB.transpose()*blockA.inverse()*blockB) * L.inverse() <<endl;


         Eigen::MatrixXd Linv0    = (unit + S.transpose()*W*S - S.transpose()*W*Ytil*  (Ytil.transpose()*W*Ytil).inverse() * Ytil.transpose() * W * S ).inverse();
         cout<<" Linv0 * orig:       "<<endl<< Linv0 * L <<endl;


         Eigen::MatrixXd Linv1    = (unit + S.transpose()*
                                     (  W - W*Ytil * (Ytil.transpose()*W*Ytil).inverse()*Ytil.transpose()*W)*S ).inverse();
         cout<<" Linv1 * orig:       "<<endl<< Linv1 * L <<endl;

         Eigen::MatrixXd monster    =  W - W*Ytil * (Ytil.transpose()*W*Ytil).inverse()*Ytil.transpose()*W ;
         Eigen::MatrixXd Linv1monster    = (unit + S.transpose()*monster*S).inverse();
         cout<<" Linv1monst * orig:       "<<endl<< Linv1monster * L <<endl;


         Eigen::MatrixXd Linv2    = unit - S.transpose()* (  (W - W*Ytil * (Ytil.transpose()*W*Ytil).inverse()*Ytil.transpose()*W).inverse() + S*S.transpose() ).inverse() *S;
         cout<<" Linv2 * orig:       "<<endl<< Linv2 * L <<endl;

         Eigen::MatrixXd LinvA2    = unit - S.transpose()*
            (  (W - W*Ytil * (blockA).inverse()*Ytil.transpose()*W).inverse() + S*S.transpose() ).inverse()
            *S;
         cout<<" LinvA2 * orig:       "<<endl<< LinvA2 * L <<endl;

         Eigen::MatrixXd Linv2neu    = unit - S.transpose()*
            ( ( W - W*Ytil * (Ytil.transpose()*W*Ytil).inverse()*Ytil.transpose()*W).inverse() + S*S.transpose() ).inverse() * S;
         cout<<" Linv2neu * orig:     "<<endl<< Linv2neu * L <<endl;

         Eigen::MatrixXd Linv2monster    = unit - S.transpose() * ( monster.inverse() + S*S.transpose() ).inverse() * S;
         cout<<" Linv2monster * orig:     "<<endl<< Linv2monster * L <<endl;

         // cout<<" monster test:     "<<endl<< monster * monster.inverse() <<endl;

         // cout<<" monsterinv test:     "<<endl<<  <<endl;

         //cout<<" unity:     "<<endl<< (Ytil*Ytil.transpose())*(Ytil*Ytil.transpose()).inverse() <<endl;

   
         { // --- ok, K1, k2 & Sigma !
            Eigen::MatrixXd SigmaInv = unit - S.transpose()* (W.inverse() + S*S.transpose() ).inverse() * S;
            Eigen::MatrixXd SigmaTest = (S.transpose()*W*S + unit);
            cout<<" SIgma test eins  1: "<<endl<<SigmaInv*SigmaTest<<endl;
            cout<<" SIgma test eins  2: "<<endl<<SigmaTest*SigmaTest.inverse()<<endl;
            cout<<" SIgma test eins  3: "<<endl<<SigmaInv*SigmaInv.inverse()<<endl;
            cout<<" SIgmaInv Meins    : "<<endl<<SigmaInv<<endl;
            cout<<" SIgmaInv Echt     : "<<endl<<SigmaTest.inverse()<<endl;
            Eigen::MatrixXd K1    = (Ytil.transpose()*W*Ytil) - Ytil.transpose()*W*S *SigmaInv * S.transpose()*W*Ytil;
            cout<<"Kinv1: "<<endl<<K1.inverse()<<endl;
            // second term:
            Eigen::MatrixXd K2    = (Ytil.transpose() * (
                                        W  - W*S*S.transpose()*W  + W*S*S.transpose() * (W.inverse() + S*S.transpose()).inverse() * S*S.transpose()*W
                                        ) * Ytil);
            cout<<"Kinv2: "<<endl<<K2.inverse()<<endl;
         }

         // {
         //    cout<<endl<<" KEN MILLER (1+STWS)^-1! "<<endl<<endl;
         //    Eigen::MatrixXd STWS2       = (S.transpose()*W*S );
         //    Eigen::MatrixXd T0       = (unit + STWS2 );
         //    Eigen::MatrixXd T0inv    = T0.inverse();
      
         //    double trT0 = STWS2.trace();
         //    Eigen::MatrixXd T1inv    = unit - 1./(trT0 + 1.) * STWS2;
         //    cout<<" T0inv * orig:       "<<endl<< T0 * T0inv <<endl;
         //    cout<<" T1inv * orig:       "<<endl<< T0 * T1inv <<endl;
         //    Eigen::MatrixXd test = T0;
         //    for ( int i = 0 ; i<T0.cols() ; i++ ) {
         //       for ( int j = 0 ; j<T0.rows() ; j++ ) {
         //          test(i,j) = T1inv(i,j) / T0inv(i,j); 
         //       }
         //    }
         ...
         // cout<<endl<<"Ytil * Ytil^T ::"<<endl;
         // cout<<Ytil*Ytil.transpose() <<endl;

         // cout<<endl<<"Komische Matrix ::"<<endl;
         // Eigen::MatrixXd komisch=W - W*Ytil*( (Ytil.transpose()*W*Ytil).inverse() ) * Ytil.transpose()*W;
         // cout<< komisch <<endl;
         // cout<<endl<<"Komische.inverse - W^-1 ::"<<endl;
         // cout<< komisch.inverse() - W.inverse() <<endl;

         // cout<<"Check blockD: "<<endl<<blockD<<endl; // ok
         // cout<<"Check SWS+1:  "<<endl<<S.transpose()*W*S + unit<<endl; //ok
         // --
         // cout<<"Check blockB: "<<endl<<blockB<<endl; // ok
         // cout<<"Check SWS+1:  "<<endl<<Ytil.transpose()*W*S<<endl; //ok

      }
      


      // --- test for non-linear approximation starts here
      {
         Eigen::MatrixXd M1tmp  = this->Mc(1,0); // linear template fit
         Eigen::MatrixXd M2tmp  = this->Mc(2,0); // second-order polynomials
         Eigen::MatrixXd M3tmp  = this->Mc(3,0); // 3rd order
         //Eigen::MatrixXd M4tmp  = this->Mc(4,0); // 4th order (don't use!)
         Eigen::MatrixXd M21tmp = this->Mc(2,1); // second-order polynomials, with interference terms

         // ybar     = Y * Mc.row(0).transpose();

         // n-parameter linear template fit:
         Eigen::VectorXd mbar  = M1tmp.row(0).transpose();
         Eigen::MatrixXd mtil  = M1tmp.bottomRows(Mc.rows()-1).transpose();
         Eigen::MatrixXd ahat1 = ((Y*mtil).transpose()*W*Y*mtil).inverse() * (Y*mtil).transpose() * W * ( Dt - Y*mbar );
         cout<<"****************************** ahat1: "<<endl<<ahat1<<endl;
         double ahut1 = ahat1(0,0);

         // n-parameter fit 2nd-order approx.:
         { // richtig!
            cout<<"1"<<endl;
            Eigen::VectorXd m2bar = M2tmp.row(0).transpose();
            cout<<"2"<<endl;
            Eigen::MatrixXd m2til = M2tmp.block(1,0,nPar,M2tmp.cols()).transpose();
            // cout<<"M2tmp: "<<endl<<M2tmp<<endl;
            // cout<<"m2til: "<<endl<<m2til.transpose()<<endl;//
            for ( int ia =0  ; ia<nPar; ia++ ) {
               cout<<"ia  "<<ia<<endl;
               m2bar         -=    M2tmp.row(1+nPar+ia).transpose() * ahat(ia,0) * ahat(ia,0);
               m2til.col(ia) += 2.*M2tmp.row(1+nPar+ia).transpose() * ahat(ia,0);
            }
            cout<<"4"<<endl;
            Eigen::MatrixXd Y2    = Y*m2til;
            cout<<"5"<<endl;
            Eigen::MatrixXd ahat2 = ((Y2).transpose()*W*Y2).inverse() * (Y2).transpose() * W * ( Dt - Y*m2bar );
            cout<<"********************* ahat2 general:: "<<endl<<ahat2<<endl;
         }

         // n-parameter fit 2nd-order approx. with interference
         if ( nPar == nParEps ) {
            
            Eigen::VectorXd m21bar = M21tmp.row(0).transpose();
            Eigen::MatrixXd m21til = M21tmp.block(1,0,nPar,M21tmp.cols()).transpose();
            for ( int ia =0  ; ia<nPar; ia++ ) {
               m21bar         -=    M21tmp.row(1+nPar+ia).transpose() * ahat(ia,0) * ahat(ia,0);
               m21til.col(ia) += 2.*M21tmp.row(1+nPar+ia).transpose() * ahat(ia,0);
            }
            ...
            for ( int j0 = 0 ; j0<nPar ; j0++ ) {
               for ( int j1 = j0+1 ; j1<nPar ; j1++ ) {
                  cout<<"Evaluating column "<<icol<<" for interference terms."<<endl;
                  m21bar         -=  1.*M21tmp.row(icol).transpose() * ahat(j0,0) * ahat(j1,0); // constant term from interference ahat0*ahat1
                  m21til.col(j0) +=     M21tmp.row(icol).transpose() * ahat(j1,0); // ahat1*a0
                  m21til.col(j1) +=     M21tmp.row(icol).transpose() * ahat(j0,0); // ahat0*a1
                  icol++;
               }
            }
            //m2bar         -=    M21tmp.row(5).transpose() * ahat(0,0) * ahat(1,0);
            
            Eigen::MatrixXd Y2    = Y*m21til;
            Eigen::MatrixXd ahat2 = ((Y2).transpose()*W*Y2).inverse() * (Y2).transpose() * W * ( Dt - Y*m21bar );
            cout<<"********************* ahat2 interference:: "<<endl<<ahat2<<endl;
         }
         ...
         { // 2D-fit, richtig !
            Eigen::VectorXd m2bar = M2tmp.row(0).transpose();
            m2bar -=  M2tmp.row(3).transpose() * ahat1(0,0) * ahat1(0,0);
            m2bar -=  M2tmp.row(4).transpose() * ahat1(1,0) * ahat1(1,0) ;
            Eigen::MatrixXd m2til = M2tmp.block(1,0,nPar,M2tmp.cols()).transpose();
            // cout<<"M2tmp: "<<endl<<M2tmp<<endl;
            // cout<<"m2til: "<<endl<<m2til.transpose()<<endl;//
            m2til.col(0) += 2.*M2tmp.row(3).transpose() * ahat(0,0);
            m2til.col(1) += 2.*M2tmp.row(4).transpose() * ahat(1,0);
            //M2tmp.row(1).transpose() + 2.*M2tmp.row(3).transpose() * ahat1(0,0)              + 2.*M2tmp.row(4).transpose() * ahat1(1,0) ;
            auto Y2    = Y*m2til;
            Eigen::MatrixXd ahat2 = ((Y2).transpose()*W*Y2).inverse() * (Y2).transpose() * W * ( Dt - Y*m2bar );
            double ahut2 = ahat2(0,0);
            cout<<"********************* ahat2: "<<endl<<ahat2<<endl;
         }

         //    { // 1D-fit, richtig !
         //    Eigen::VectorXd m2bar = M2tmp.row(0).transpose() -    M2tmp.row(2).transpose() * ahut1 * ahut1;
         //    Eigen::VectorXd m2til = M2tmp.row(1).transpose() + 2.*M2tmp.row(2).transpose() * ahut1 ;
         //    auto Y2    = Y*m2til;
         //    Eigen::MatrixXd ahat2 = ((Y2).transpose()*W*Y2).inverse() * (Y2).transpose() * W * ( Dt - Y*m2bar );
         //    double ahut2 = ahat2(0,0);

         
         //    auto m3bar = M3tmp.row(0).transpose() - M3tmp.row(2).transpose() * ahut1 * ahut1 - 2. *M3tmp.row(3).transpose() * pow(ahut1,3);
         //    auto m3til = M3tmp.row(1).transpose() + 2. * M3tmp.row(2).transpose() * ahut1  + 3. * M3tmp.row(3).transpose() * ahut1 * ahut1   ;
         //    auto Y3    = Y*m3til;
         //    Eigen::MatrixXd ahat3 = ((Y3).transpose()*W*Y3).inverse() * (Y3).transpose() * W * ( Dt - Y*m3bar );
         //    double ahut3 = ahat3(0,0);

         //    //auto m21bar = M2tmp.row(0).transpose() - M2tmp.row(2).transpose() * ahat1(0,0) * ahat1(0,0) - M2tmp.row(2).transpose() * ahat1(0,0) * ahat1(1,0) ;
         //    // auto m21til = M21tmp.row(1).transpose() + 2. * M21tmp.row(2).transpose() * ahut1  + 3. * M21tmp.row(3).transpose() * ahut1 * ahut1   ;
         //    // auto Y21    = Y*m21til;
         //    // Eigen::MatrixXd ahat21 = ((Y21).transpose()*W*Y21).inverse() * (Y21).transpose() * W * ( Dt - Y*m21bar );
         //    // double ahut21 = ahat21(0,0);


         //    // for ( size_t j = 0 ; j<Gamma.size() ;j++ )  // apply gamma factor first to ahat
         //    //    this->ahat(j) = pow( ahat(j), 1./this->Gamma[j] ); 
      
         //    cout<<"ahut1:  "<< ahut1<<"\tDelta: "<< ahut1 - ahat(0) <<endl;
         //    cout<<"ahut2:  "<< ahut2<<"\tDelta: "<< ahut2 - ahat(0) <<endl;
         //    cout<<"ahut3:  "<< ahut3<<"\tDelta: "<< ahut3 - ahat(0) <<endl;
         //    cout<<"======== with gamma:"<<endl;
         //    cout<<"ahut1:  "<<  pow( ahut1, 1./this->Gamma[0] ) <<endl;
         //    cout<<"ahut2:  "<<  pow( ahut2, 1./this->Gamma[0] ) <<endl;
         //    cout<<"ahut3:  "<<  pow( ahut3, 1./this->Gamma[0] ) <<endl;
         //    cout<<"======== ndim ohne gamma:"<<endl;
         //    cout<<"ahut1:  "<<  ahat1 <<endl;
         //    cout<<"ahut2:  "<<  ahat2 <<endl;
         //    cout<<"ahut3:  "<<  ahat3 <<endl;
         //    cout<<"======== with gamma:"<<endl;
         //    cout<<"ahut1:  "<< ahut1<<"\tDelta: "<< pow( ahut1, 1./this->Gamma[0] ) - a0 <<endl;
         //    cout<<"ahut2:  "<< ahut2<<"\tDelta: "<< pow( ahut2, 1./this->Gamma[0] ) - a0 <<endl;
         //    cout<<"ahut3:  "<< ahut3<<"\tDelta: "<< pow( ahut3, 1./this->Gamma[0] ) - a0 <<endl;
         // }

      }

      // {
      //    Eigen::MatrixXd M1tmp = M;
      //    Eigen::MatrixXd M2tmp = Eigen::MatrixXd::Zero( M.rows(), 3);;
      //    Eigen::MatrixXd M3tmp = Eigen::MatrixXd::Zero( M.rows(), 4);;
      //    Eigen::MatrixXd M4tmp = Eigen::MatrixXd::Zero( M.rows(), 5);;
      ...
      //       M1tmp( i , 1 ) = pow(M(i,1), 1*this->Gamma[0]);
      //       M2tmp( i , 1 ) = pow(M(i,1), 1*this->Gamma[0]);
      //       M3tmp( i , 1 ) = pow(M(i,1), 1*this->Gamma[0]);
      //       M4tmp( i , 1 ) = pow(M(i,1), 1*this->Gamma[0]);

      //       M2tmp( i , 2 ) = pow(M(i,1), 2*this->Gamma[0]);
      //       M3tmp( i , 2 ) = pow(M(i,1), 2*this->Gamma[0]);
      //       M4tmp( i , 2 ) = pow(M(i,1), 2*this->Gamma[0]);

      //       M3tmp( i , 3 ) = pow(M(i,1), 3*this->Gamma[0]);
      //       M4tmp( i , 3 ) = pow(M(i,1), 3*this->Gamma[0]);

      //       M4tmp( i , 4 ) = pow(M(i,1), 4*this->Gamma[0]);
      //    }
      //    M1tmp = (M1tmp.transpose() * M1tmp).inverse() * M1tmp.transpose();
      //    M2tmp = (M2tmp.transpose() * M2tmp).inverse() * M2tmp.transpose();
      //    M3tmp = (M3tmp.transpose() * M3tmp).inverse() * M3tmp.transpose();
      //    M4tmp = (M4tmp.transpose() * M4tmp).inverse() * M4tmp.transpose();

      //    cout<<"M1tmp: "<<endl<<M1tmp<<endl;
      //    cout<<"M1tmp neu: "<<endl<<this->Mc(1,0)<<endl;

      //    Eigen::MatrixXd M2tmp2 = this->Mc(2);
      //    cout<<"M2tmp:  "<<endl<<M2tmp<<endl;
      //    cout<<"M2tmp2: "<<endl<<M2tmp2<<endl;

      //    cout<<"Interferecne tests:"<<endl;
      //    this->Mc(1,1);
      //    this->Mc(2,1);
      //    this->Mc(3,1);

      //    cout<<"M3tmp: "<<endl<<M3tmp<<endl;
      //    cout<<"M3tmp neu: "<<endl<<this->Mc(3,0)<<endl;

      //    cout<<"M4tmp: "<<endl<<M3tmp<<endl;
      //    cout<<"M4tmp neu: "<<endl<<this->Mc(4,0)<<endl;


      //    // cout<<"M1hut: "<<endl<<M1tmp<<endl;
      //    // cout<<"M2hut: "<<endl<<M2tmp<<endl;
      //    // cout<<"M3hut: "<<endl<<M3tmp<<endl;
      //    // cout<<"M4hut: "<<endl<<M4tmp<<endl;


      //    // ybar     = Y * Mc.row(0).transpose();
      //    auto mbar = M1tmp.row(0).transpose();
      //    auto mtil = M1tmp.row(1).transpose();
      //    Eigen::MatrixXd ahat1 = ((Y*mtil).transpose()*W*Y*mtil).inverse() * (Y*mtil).transpose() * W * ( Dt - Y*mbar );
      //    double ahut1 = ahat1(0,0);

      //    auto m2bar = M2tmp.row(0).transpose() - M2tmp.row(2).transpose() * ahut1 * ahut1;
      //    auto m2til = M2tmp.row(1).transpose() + 2.*M2tmp.row(2).transpose() * ahut1 ;
      //    auto Y2    = Y*m2til;
      //    Eigen::MatrixXd ahat2 = ((Y2).transpose()*W*Y2).inverse() * (Y2).transpose() * W * ( Dt - Y*m2bar );
      //    double ahut2 = ahat2(0,0);
      ...
      if ( nPar==1 )  
      {
         Eigen::MatrixXd M1tmp  = this->Mc(1,0); // linear template fit
         Eigen::MatrixXd M2tmp  = this->Mc(2,0); // second-order polynomials
         Eigen::MatrixXd M21tmp = this->Mc(2,1); // second-order polynomials, with interference 

         cout<<"M1tmp:  "<<endl<<M1tmp<<endl;
         cout<<"M2tmp:  "<<endl<<M2tmp<<endl;
         cout<<"M21tmp: "<<endl<<M21tmp<<endl;

         Eigen::MatrixXd mbar = M1tmp.row(0).transpose();
         Eigen::MatrixXd mtil = M1tmp.row(1).transpose();
         Eigen::MatrixXd ahat1 = ((Y*mtil).transpose()*W*Y*mtil).inverse() * (Y*mtil).transpose() * W * ( Dt - Y*mbar );
         double ahut1 = ahat1(0,0);


         Eigen::MatrixXd m2bar = M2tmp.row(0).transpose() - M2tmp.row(2).transpose() * ahut1 * ahut1;
         Eigen::MatrixXd m2til = M2tmp.row(1).transpose() + 2.*M2tmp.row(2).transpose() * ahut1 ;
         Eigen::MatrixXd Y2    = Y*m2til;
         Eigen::MatrixXd ahat2 = ((Y2).transpose()*W*Y2).inverse() * (Y2).transpose() * W * ( Dt - Y*m2bar );
         double ahut2 = ahat2(0,0);

         // --- error propagation of m (starts here)
         double deltam2   = 0;
         double deltam2sq = 0;
         double deltam3 = 0;
         double deltam4 = 0;
         Eigen::MatrixXd dmbar2 = m2bar-mbar;
         Eigen::MatrixXd dmtil2 = m2til-mtil;
         Eigen::VectorXd dmall = Eigen::VectorXd::Zero(mbar.rows());
         //vector<double> dmall(mbar.rows());
         for ( int j = 0 ; j<mbar.rows() ; j++ ) {
            // mbar:
            Eigen::VectorXd j1 = Eigen::VectorXd::Zero(mbar.rows());
            j1(j) = 1.;
            // mbar
            dmall(j) = (-F*Y*j1)(0,0) * dmbar2(j,0);
            // // mtilde
            dmall(j) += (Dinv*( (Y*j1).transpose()*W*d_ybar - ((Y*j1).transpose()*W*Y*mtil + (Y*mtil).transpose()*W*Y*j1) * ahut1 ))(0,0) * dmtil2(j,0);
            deltam2  += dmall[j];
            deltam2sq += dmall[j]*dmall[j];

            // deltam2 += (-F*Y*j1)(0,0) * dmbar2(j,0);
            // deltam2 += (Dinv*( (Y*j1).transpose()*W*d_ybar - ((Y*j1).transpose()*W*Y*mtil + (Y*mtil).transpose()*W*Y*j1) * ahut1 ))(0,0) * dmtil2(j,0);
         }
         cout<<"deltam: "<<deltam2
             <<"\t uncorr: "<<sqrt(deltam2sq)
             <<"\t m-m2: "<<ahut2-ahut1<<endl;
         cout<<"dmall: "<< dmall.transpose()<<endl;
      }
      ...
      if ( nPar<=2 )  
      {
         ...
         for ( int j0 = 0 ; j0<nPar ; j0++ ) {
            for ( int j1 = j0+1 ; j1<nPar ; j1++ ) {
               cout<<"Evaluating column "<<icol<<" for intereference terms."<<endl;
               m21bar         -=  1.*M21tmp.row(icol).transpose() * ahat1(j0,0) * ahat1(j1,0); // contant term from interference ahat0*ahat1
               M21til.col(j0) +=     M21tmp.row(icol).transpose() * ahat1(j1,0); // ahat1*a0
               M21til.col(j1) +=     M21tmp.row(icol).transpose() * ahat1(j0,0); // ahat0*a1
               icol++;
            }
         }
         //m2bar         -=    M21tmp.row(5).transpose() * ahat(0,0) * ahat(1,0);
         Eigen::MatrixXd Y21    = Y*M21til;
         Eigen::MatrixXd ahat21 = ((Y21).transpose()*W*Y21).inverse() * (Y21).transpose() * W * ( Dt - Y*m21bar );




         // pol2 NO interference
         Eigen::VectorXd m2bar = M2tmp.row(0).transpose();
         Eigen::MatrixXd M2til = M2tmp.block(1,0,nPar,M2tmp.cols()).transpose();
         for ( int ia =0  ; ia<nPar; ia++ ) {
            m2bar         -=    M2tmp.row(1+nPar+ia).transpose() * ahat(ia,0) * ahat(ia,0);
         ...
         // --- error propagation of m (starts here)
         //  ---------------------------------------------------------------- //
         Eigen::MatrixXd dmbar2 = m21bar-mbar;
         Eigen::MatrixXd dMtil2 = M21til-Mtil;
         
         Eigen::VectorXd sigmam1 = Eigen::VectorXd::Zero(ahat1.rows());
         Eigen::VectorXd sigmam2 = Eigen::VectorXd::Zero(ahat1.rows());
         // mbar
         for ( int j = 0 ; j<mbar.rows() ; j++ ) {
            Eigen::VectorXd j0 = Eigen::VectorXd::Zero(mbar.rows());
            j0(j) = 1.;
            // mbar
            sigmam1 +=  (-F*Y*j0) * dmbar2(j,0);
         }
         // Mtil
         for ( int j = 0 ; j<mbar.rows() ; j++ ) {
            for ( int k = 0 ; k<nPar ; k++ ) {
               // Eigen::MatrixXd j1 = Eigen::MatrixXd::Zero(Mtil.rows(),Mtil.cols());
               // j1(j,k) = 1.;
               // Eigen::MatrixXd Yj2 = Y*j1;
               //sigmam2 += (Dinv*( (Y*j1).transpose()*W*(Dt - Y*mbar) - ((Y*j1).transpose()*W*Y*Mtil + (Y*Mtil).transpose()*W*Y*j1) * ahat1 )) * dMtil2(j,k);
               Eigen::MatrixXd Yj2 = Eigen::MatrixXd::Zero(Y.rows(),nPar); // Yj2 = Y*j1
               Yj2.col(k) += Y.col(j);
               sigmam2 += (Dinv*( (Yj2).transpose()*W*(Dt - Y*mbar) - ((Yj2).transpose()*W*Y*Mtil + (Y*Mtil).transpose()*W*Yj2) * ahat1 )) * dMtil2(j,k);
                  
            }
         }
         cout<<"-============================ "<<endl;
         cout<<  (sigmam1+sigmam2)<<endl
             <<"\t (0) m-m2: "<<ahat21(0,0)-ahat1(0,0)
             <<endl;
         if( nPar == 2 )
            cout<<"\t (1) m-m2: "<<ahat21(1,0)-ahat1(1,0)<<endl;
         ...
         // ------------------------------------------------------------------- //
         if( nPar == 1 ) {
            // Eigen::MatrixXd dmYbar2 = Y*(m2bar-mbar);
            // Eigen::MatrixXd dmYtil2 = Y*(m2til-mtil);
            Eigen::MatrixXd dmYbar2 = Y*dmbar2;//(m2bar-mbar);
            Eigen::MatrixXd dmYtil2 = Y*dMtil2;;
            Eigen::VectorXd dmYall = Eigen::VectorXd::Zero(Dt.rows());
            //vector<double> dmall(mbar.rows());
            double deltaYm2 = 0;
            double deltaYm2sq = 0;
            for ( int i = 0 ; i<Dt.rows() ; i++ ) {
               // Ymbar:
               Eigen::VectorXd i1 = Eigen::VectorXd::Zero(Dt.rows());
               i1(i) = 1.;
               // Ymbar
               dmYall(i) = (-F*i1)(0,0) * dmYbar2(i,0);
               // mtilde
               dmYall(i) += (Dinv*( i1.transpose()*W*d_ybar - ( i1.transpose()*W*Y*Mtil + (Y*Mtil).transpose()*W*i1) * ahut1 ))(0,0) * dmYtil2(i,0);
               deltaYm2 += dmYall[i];
               deltaYm2sq += dmYall[i] * dmYall[i];
            }
            cout<<"deltaYm: "<<deltaYm2
                <<"\tuncorr: "<<sqrt(deltaYm2sq)
                <<endl;
            cout<<"dmYall:  "<< dmYall.transpose()<<endl;
         }
      
         // apply gamma factor
         // auto MT = Mtmp.transpose();
         // return (MT*Mtmp).inverse()*MT;
      }
   ...
   */
   ...
   if ( this->SysY.size() ) {
      ...
      for ( int i = 0 ; i<dYtmp.rows() ; i++ ) {
         for ( int j = 0 ; j<dYtmp.cols() ; j++ ) {
            ...
            // if ( i==2 && j==1 ) {
            //    cout<<"Mtil  "<<endl<<Mtil<<endl;
            //    cout<<"d1ij  "<<endl<<d1ij<<endl;
            //    cout<<"d1M:  "<<endl<<d1M<<endl;
            //    cout<<"d1M0: "<<endl<<d1M0<<endl;
            //    cout<<"YtS:  "<<endl<<YtS<<endl;
            //    cout<<"E:    "<<endl<<E<<endl;
            //    cout<<"dYij_1_2:  "<<endl<<dYij_1_2<<endl;
            //    cout<<"dYij_3:    "<<endl<<dYij_3<<endl;
            //    cout<<"dYij_4:    "<<endl<<dYij_4<<endl;
            //    cout<<"dYij_X1:    "<<endl << YtSTW * d1M0<<endl;
            //    cout<<"dYij_X2:    "<<endl << d1M0.transpose() * W * YtS <<endl;
            //    cout<<"dYij_X1*a:    "<<endl << YtSTW * d1M0 * ahat<<endl;
            //    cout<<"dYij_X2*a:    "<<endl << d1M0.transpose() * W * YtS * ahat<<endl;
            //    cout<<"dYij:      "<<endl<<dYijs[i][j]<<endl;
            // }
         }
      }
   ...
   // // --- old code
   // // // bTW = YmtilT*W
   // // double Denom = (bTW*Ymtil)(0,0);
   // // auto D  = bTW * (1./Denom); // fit response
   // // auto F   = D.transpose();
   // // auto Fbar = ( W * Ymbar) * (1./Denom);
   // // double mt = pow( (D*Dt - FT*Ymbar )(0,0), 1./fDelta);
   // // double d_mt = sqrt((FT*V*F)(0,0));
   // // double d_mt_2 = 0;
