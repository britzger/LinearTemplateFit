...
int example_CMSinclusivejets_MSTW_CMSpaper() {
   ...
   map < string, vector<double> > templates_CT10nlo = read_input_table("data/CMS_MSTWnlo_all.txt",16); string PDF="MSTW"; // almost ok, if rescaling, and sign of PDF uncertainty is droped
   ...
   for ( int i = 0; i<data.size()+1; i++ ) {
      //  The JES, unfolding, and luminosity uncertainties are treated as multiplicativ [paper]
      //input_table["stat"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      //input_table["uncor"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      //input_table["npcorerr"   ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["CMS_lumierr"][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["unf_err"    ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC0"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC1"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC3"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC4"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC5"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC6"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC7"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC8"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC9"       ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC10"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC11"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC12"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC13"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC14"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC15"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC2a"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC2b"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC2c"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC2d"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
      input_table["JEC2e"      ][i]*= templates_CT10nlo[rescale][i] / data[i];
   }
   ...
   ltf_CMS.AddErrorPercent( "npcorerr"    , input_table["npcorerr"   ], 1. ,  LTF::Uncertainty::External );
   ...
   if ( PDF == "CT10" ) pdferrors = read_input_table("data/CT10.errorAbsolut.txt",26);
   if ( PDF == "MSTW" ) pdferrors = read_input_table("data/MSTW2008nlo68cl.errorFabsAbsolut.txt",20);
...
#if defined __WITH_ROOT__ || defined __CLING__
   plotLiTeFit(fit,bins);//, const vector<double> bins)
#endif
   return 0;
